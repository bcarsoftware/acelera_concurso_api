...
study_tips_route = APIRouter(prefix="/study-tips", tags=["Study Tips"])

controller_study_tips: StudyTipsControllerInterface = StudyTipsController()


@exception_handler
@study_tips_route.post("")
@authenticated
...
@exception_handler
@study_tips_route.patch("/{study_tip_id}/user/{user_id}")
@authenticated
...
@exception_handler
@study_tips_route.delete("/{user_id}")
@authenticated
